    ...
    return (
        <>
            <div className="flex flex-row justify-between items-center">
                <div>
                    <button onClick={() => setOpen(true)} className="flex flex-row items-center gap-[5px] bg-[#eff0e8] py-[8px] px-[20px] font-[600]">
                ...
                <div className="flex flex-row items-center gap-[30px]">
                    <div className="relative">
                        <Icon icon="lets-icons:search-alt-light" width={25} className="absolute top-[5px] left-[4px]" />
                        <input
                            placeholder="Tìm kiếm theo tên"
                            value={searchVal}
                            onInput={handleSearch}
                            className="border border-[#e9eaeb] placeholder:text-[14px] placeholder:font-[400] pl-[30px] h-[36px] pr-[10px] rounded-[5px] outline-none" />
                    </div>
                    <div className="flex flex-row items-center gap-[10px]">
                        <span>Sort by:</span>
                        <Select
                            className="w-[180px] custom-sort h-[36px]"
                            showSearch
                            placeholder="Tất cả"
                ...
                {data?.products?.map((item, index) => {
                    ...
                    return (
                        <div onClick={() => router.push(`/shop/${item._id}`)} className="cursor-pointer flex flex-col gap-[10px]">
                                ...
                                >
                                    {item?.productImages?.map((img, imgKey) => (
                                        <SwiperSlide key={imgKey}>
                                            <Image src={img} width={352} height={510} className='h-[510px] rounded-[10px]' alt={`Image ${imgKey}`} />
                                        </SwiperSlide>
                                    ))}
                                </Swiper>
                            ...
                            <div className='cursor-pointer' onClick={() => router.push(`/shop/${item._id}`)}>
                                <div className="flex flex-row gap-[5px] items-center">
                                    <div className="flex flex-row gap-[2px]">
                                        {Number(rating) > 0 && Array(Math.floor(Number(rating))).fill(null).map((_, index) => (
                                            <Icon key={index} icon="tdesign:star-filled" />
                                        ))}
                                    </div>
                                    <div>{Number(rating) > 0 ? Number(rating) : 0 + ' lượt đánh giá'}</div>
                                    <div>{Number(item.ratingCount) > 0 && (
                                        <b className='font-[500]'>({item.ratingCount})</b>
                                    )}</div>
                                </div> 
                                <h1 className='text-[18px] font-[500]'>{item?.productName}</h1>
                            </div>
                            <div>
                            {priceIndices.length === 0 || priceIndices[0] !== index ? (
                                ...
                                item?.productOption[priceIndices[1]]?.productPrice && (
                                    priceIndices[0] === index && formatCurrency(item.productOption[priceIndices[1]].productPrice)
                                )
                            )}
                            </div>
                            <div className='flex flex-row gap-[10px]'>
                                {item?.productOption.map((option: any, optionIndex: number) => {
                                    const isActive = priceIndices[1] === optionIndex && priceIndices[0] === index;
                                    return (
                                        <button 
                                            onClick={() => handleSelectPriceIndex(index, optionIndex)}
                                            className={`border border-black rounded-[3px] px-[10px] py-[2px] text-black 
